...
describe("prisma schema foundation (infra + users + CRM core + orders + supply inventory + payments/finance + logistics/fulfillment contract baseline)", () => {
  it("contains approved infra/system models, users models, and logistics/fulfillment contract baseline models", () => {
    ...
    expect(schema).toContain("model SystemIntegrationInboxEvent");
    expect(schema).toContain("model SystemNotificationDispatch");
    expect(schema).toContain("model AuditLogRecord");
    expect(schema).toContain("enum RecordStatus");
    expect(schema).toContain("enum IntegrationInboxStatus");
    expect(schema).toContain('RECEIVED  @map("received")');
    expect(schema).toContain('PROCESSED @map("processed")');
    expect(schema).toContain("enum NotificationDispatchStatus");
    expect(schema).toContain('QUEUED @map("queued")');
    expect(schema).toContain('SENT   @map("sent")');
    expect(schema).toContain("sourceSystem");
    expect(schema).toContain('@map("source_system") @db.VarChar(32)');
    expect(schema).toContain("externalEventId");
    expect(schema).toContain('@map("external_event_id") @db.VarChar(128)');
    expect(schema).toContain("@@unique([sourceSystem, externalEventId])");
    expect(schema).toContain("@@index([status, receivedAt])");
    expect(schema).toContain('@@map("integration_inbox_events")');
    expect(schema).toContain("targetRef");
    expect(schema).toContain('@map("target_ref") @db.VarChar(255)');
    expect(schema).toContain("queuedAt");
    expect(schema).toContain('@map("queued_at")');
    expect(schema).toContain('@@map("notification_dispatches")');
    expect(schema).toContain("@@index([channel, status, queuedAt])");
    expect(schema).toContain("@@index([eventType, targetRef])");
    ...
    expect(schema).toContain("model CrmDealFollowUp");
    expect(schema).toContain("model CrmDealCommunication");
    expect(schema).toContain("model CrmClientMergeCase");
    ...
    expect(schema).toContain("addressText");
    expect(schema).toContain('@map("address_text")');
    expect(schema).toContain("installerReferralComment");
    expect(schema).toContain('@map("installer_referral_comment")');
    expect(schema).toContain("designerReferralComment");
    expect(schema).toContain('@map("designer_referral_comment")');
    expect(schema).toContain("nextContactAt");
    expect(schema).toContain('@map("next_contact_at")');
    expect(schema).toContain("lostReasonCode");
    expect(schema).toContain('@map("lost_reason_code")');
    expect(schema).toContain("stuckReasonCode");
    expect(schema).toContain('@map("stuck_reason_code")');
    expect(schema).toContain("isStuck");
    expect(schema).toContain('@map("is_stuck")');
    expect(schema).toContain('@@map("deal_follow_ups")');
    expect(schema).toContain("ownerUserId");
    expect(schema).toContain('@map("owner_user_id")');
    expect(schema).toContain('@@map("deal_communications")');
    expect(schema).toContain("authorUserId");
    expect(schema).toContain('@map("author_user_id")');
    expect(schema).toContain('@@map("client_merge_cases")');
    expect(schema).toContain("primaryClientId");
    expect(schema).toContain('@map("primary_client_id")');
    expect(schema).toContain("candidateClientId");
    expect(schema).toContain('@map("candidate_client_id")');
